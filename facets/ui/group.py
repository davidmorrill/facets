...
from facets.core_api \
    import FacetError, ReadOnly, Constant, Delegate, List, Str, Range, Enum, \
           Instance, Bool
...
# Delegate facet to the object being "shadowed"
...
class Group ( ViewSubElement ):
    ...
    # * 'fold': Sub-groups are displayed as VerticalNotebook tabs that can be
    #   individually expanded and collapsed.
    ...
    # * 'toolbar': Sub-groups are displayed within a ToolbarControl, which
    #   manages large numbers of controls using a mini-scrollbar and also
    #   provides an 'edit' mode that allows the user to re-arrange and hide
    #   controls as needed.
    layout = Layout
    ...
    # that any facet value is edited in the display. Therefore, you can use
    # **visible_when** conditions to hide or show groups in response to user
    # input.
    visible_when = Str

    # Pre-condition for enabling the group. If the expression evaluates to
    # False, the group is disabled, that is, none of the widgets accept input.
    # All **enabled_when** conditions are checked each time that any facet value
    ...
    def __init__ ( self, *values, **facets ):
        ...
        for value in values:
            if isinstance( value, basestring ) and (value[ 0: 1 ] in '-|'):
                # Parse Group facet options if specified as a string:
                self._parse( value )
        ...
        for value in values:
            if isinstance( value, ViewSubElement ):
                ...
                if value[ 0: 1 ] in '-|':
                    # We've already parsed Group facet options above:
                    pass
    ...
    def _dock_theme_default ( self ):
        from facets.ui.dock.dock_window_theme_factory import theme_factory
    ...
    def __repr__ ( self ):
        ...
        """
        return self._repr_format( self.content, 'orientation', 'show_border',
            'show_labels', 'show_left', 'selected', 'id', 'object', 'label',
            'style', 'layout'
        )
...
class HGroup ( Group ):
    ...
    # Override standard Group facet defaults to give it horizontal group
    # behavior:
...
class VGroup ( Group ):
    ...
    # Override standard Group facet defaults to give it vertical group behavior:
...
class VGrid ( VGroup ):
    ...
    # Override standard Group facet defaults to give it grid behavior:
    columns = 2
...
class HFlow ( HGroup ):
    ...
    # Override standard Group facet defaults to give it horizontal flow
    # behavior:
    layout      = 'flow'
    show_labels = False
...
class VFlow ( VGroup ):
    ...
    # Override standard Group facet defaults to give it vertical flow behavior:
    layout      = 'flow'
    show_labels = False
...
class VFold ( VGroup ):
    ...
    # Override standard Group facet defaults to give it vertical folding group
    # behavior:
    layout      = 'fold'
    show_labels = False
...
class HSplit ( Group ):
    ...
    # Override standard Group facet defaults to give it horizontal splitter
    # behavior:
    layout      = 'split'
    orientation = 'horizontal'
...
class VSplit ( Group ):
    ...
    # Override standard Group facet defaults to give it vertical splitter
    # behavior:
    layout      = 'split'
    orientation = 'vertical'
...
class HToolbar ( Group ):
    """ A horizontal group of toolbar controls with a mini-scrollbar and the
        ability to re-arrange and hide/show controls as needed.
    """

    #-- Facet Definitions ------------------------------------------------------

    # Override standard Group facet defaults to give it horizontal toolbar
    # behavior:
    layout      = 'toolbar'
    orientation = 'horizontal'
    padding     = 2   # used for ToolbarControl.margin

    # The spacing between adjacent toolbar controls:
    spacing = Range( 0, 32, 4 )
...
class VToolbar ( HToolbar ):
    """ A vertical group of toolbar controls with a mini-scrollbar and the
        ability to re-arrange and hide/show controls as needed.
    """

    #-- Facet Definitions ------------------------------------------------------

    # Override standard HToolbar facet defaults to give it vertical toolbar
    # behavior:
    orientation = 'vertical'
    alignment   = 'fill'
    spacing     = 2
...
class Tabbed ( Group ):
    ...
    # Override standard Group facet defaults to give it tabbed notebook
    # behavior:
    layout  = 'tabbed'
    springy = True
...
class StatusBar ( HGroup ):
    """ A horizontal group for use in creating status bars.
    """

    #-- Facet Definitions ------------------------------------------------------

    # Define a custom item them:
    item_theme = ATheme

    # Do not displaying any labels:
    show_labels = Constant( False )

    #-- Facet Default Values ---------------------------------------------------

    def _item_theme_default ( self ):
        return Theme( '@xform:b?L30', content = ( 4, 2 ) )
...
class ShadowGroup ( Group ):
    ...
    # The spacing between adjacent toolbar controls:
    spacing = ShadowDelegate

    # The alignment of the toolbar controls along the non-layout axis:
    alignment = ShadowDelegate
    ...
    def get_content ( self, allow_groups = True ):
        """ Returns the contents of the Group within a specified context for
            ...
            This method makes sure that all Group types are of the same type
            (i.e., Group or Item) and that all Include objects have been
            replaced by their substituted values.
        """
