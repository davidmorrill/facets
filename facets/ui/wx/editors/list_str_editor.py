"""
Facets UI editor for editing lists of strings.
"""
...
from facets.api \
    import Str, Int, Enum, List, Bool, Instance, Any, Event, FacetListEvent, \
           Property, BasicEditorFactory

from facets.ui.list_str_adapter \
...
class wxListCtrl ( wx.ListCtrl ):
    ...
    def OnGetItemImage ( self, index ):
        ...
        editor = self._editor
        if editor._is_auto_add( index ):
            image = editor.ls_adapter.get_default_image( editor.object,
                                                         editor.name )
        else:
            image = editor.ls_adapter.get_image( editor.object, editor.name,
                                                 index )
    ...
    def OnGetItemText ( self, index, column ):
        ...
        if editor._is_auto_add( index ):
            return editor.ls_adapter.get_default_text( editor.object,
                                                       editor.name )

        return editor.ls_adapter.get_text( editor.object, editor.name, index )
...
class _ListStrEditor ( Editor ):
    ...
    # initial state of the editor factory 'multi_select' facet):
    selected       = Any
    multi_selected = List

    # The current set of selected item indices (which one is used depends upon
    # the initial state of the editor factory 'multi_select' facet):
    selected_index         = Int
    ...
    ls_adapter = Instance( ListStrAdapter )
    ...
    def init ( self, parent ):
        ...
        self.ls_adapter = factory.adapter
        self.sync_value( factory.adapter_name, 'ls_adapter', 'from' )
        ...
        # Synchronize other interesting facets as necessary:
        ...
        self.on_facet_set( self._refresh, 'ls_adapter.+update',
                           dispatch = 'ui' )
    ...
    def update_editor ( self ):
        ...
        n       = self.ls_adapter.len( self.object, self.name )
    ...
    def _selected_index_set ( self, selected_index ):
        """ Handles the editor's 'selected_index' facet being changed.
        """
        if not self._no_update:
    ...
    def _multi_selected_set ( self, selected ):
        """ Handles the editor's 'multi_selected' facet being changed.
        """
        if not self._no_update:
            values = self.value
            try:
                self._multi_selected_indices_set( [ values.index( item )
                                                    for item in selected ] )
            except:
                pass


    def _multi_selected_items_set ( self, event ):
        """ Handles the editor's 'multi_selected' facet being modified.
        """
        values = self.values
        try:
            self._multi_selected_indices_items_changed( FacetListEvent( 0,
    ...
    def _multi_selected_indices_set ( self, selected_indices ):
        """ Handles the editor's 'multi_selected_indices' facet being changed.
        """
        if not self._no_update:
            control  = self.control
    ...
    def _multi_selected_indices_items_set ( self, event ):
        """ Handles the editor's 'multi_selected_indices' facet being modified.
        """
        control = self.control
    ...
    def _begin_label_edit ( self, event ):
        ...
        if ((not self._is_auto_add( index )) and
            (not self.ls_adapter.get_can_edit( self.object, self.name,
                                               index ))):
            event.Veto()
    ...
    def _item_selected ( self, event ):
        ...
        try:
            get_item         = self.ls_adapter.get_item
            object, name     = self.object, self.name
    ...
    def _item_activated ( self, event ):
        ...
        if 'edit' in self.factory.operations:
            ...
            self.activated = self.ls_adapter.get_item( self.object, self.name,
    ...
    def _right_clicked ( self, event ):
        ...
        self.right_clicked = self.ls_adapter.get_item( self.object, self.name,
                                                       index )
    ...
    def wx_drag_over ( self, x, y, data, drag_result ):
        ...
        if ((index != -1) and
            self.ls_adapter.get_can_drop( self.object, self.name, index,
                                          data )):
            return drag_result
    ...
    def _search_for_string ( self, increment = 0 ):
        ...
        get_text = self.ls_adapter.get_text
        search   = self.search
        object   = self.object
        name     = self.name
    ...
    def _copy_current ( self ):
        ...
        if len( selected ) == 1:
            ...
            if index < self.item_count:
                from facets.ui.wx.util.clipboard import clipboard

                clipboard.data = self.ls_adapter.get_text( self.object,
                                                           self.name, index )
        ...
        if ('insert' in ops) and ('delete' in ops):
            ...
            if len( selected ) == 1:
                ...
                if index < self.item_count:
                    ...
                    clipboard.data = self.ls_adapter.get_text(
                                                 self.object, self.name, index )
                    self.index = index
                    self.ls_adapter.delete( self.object, self.name, index )
    ...
    def _is_auto_add ( self, index ):
        ...
        return (self.factory.auto_add and
                (index >= self.ls_adapter.len( self.object, self.name )))
    ...
    def _set_text_current ( self, index, text, insert = False ):
        ...
        """
        if text.strip() != '':
            object, name, adapter = self.object, self.name, self.ls_adapter
...
class ListStrEditor ( BasicEditorFactory ):
    ...
    # The optional extended name of the facet to synchronize the selection
    # values with:
    selected = Str
    ...
    # The optional extended name of the facet to synchronize the activated value
    # with:
    activated = Str

    # The optional extended name of the facet to synchronize the activated
    # value's index with:
    activated_index = Str

    # The optional extended name of the facet to synchronize the right clicked
    # value with:
    right_clicked = Str
    ...
    # The optional extended name of the facet containing the editor title:
    title_name = Str
    ...
    # The optional extended name of the facet containing the adapter:
    adapter_name = Str
