...
from facets.api \
    import HasStrictFacets, Any, Dict, Bool, Tuple, Int, List, Instance, Str, \
           Event, Enum, FacetError, View, TreeNode, ObjectTreeNode, Editor,   \
           MultiTreeNode, EditorFactory, toolkit
...
from facets.ui.pyface.image_list \
    import ImageList
...
try:
    from facets.ui.wx.util.drag_and_drop import PythonDropSource, \
                                                PythonDropTarget
except:
...
class TreeEditor ( EditorFactory ):
    ...
    # The optional extended facet name of the facet to synchronize with the
...
class SimpleEditor ( Editor ):
    ...
    def init ( self, parent ):
        ...
        wx.EVT_TREE_SEL_CHANGED(      tree, tid, self._on_tree_sel_modified )
        wx.EVT_TREE_BEGIN_DRAG(       tree, tid, self._on_tree_begin_drag )
        wx.EVT_TREE_BEGIN_LABEL_EDIT( tree, tid,
            ...
            if node is not None:
                ...
                if self.factory.hide_root or self._has_children( node, object ):
                    ...
                    if not self.factory.hide_root:
                        ...
                        # Note: For some reason, explicitly calling SelectItem
                        # generates two wx events (which means the same panel
                        # may need to get created twice). To work around this,
                        # we tell the event handler to ignore the events, then
                        # we explicitly call the event handler ourselves:
                        self._locked = True
                        tree.SelectItem( nid )
                        self._locked = False
                        self._on_tree_sel_modified()
    ...
    def _selection_set ( self, selection ):
    ...
    def _get_icon ( self, node, object, is_expanded = False ):
        ...
        if isinstance( icon_name, AnImageResource ):
            file_name = icon_name
    ...
    def _on_tree_sel_modified ( self, event = None ):
        ...
        if editor is not None:
            ...
            if object is not None:
                ...
                if view is None:
                    view = object.facet_view()

                ui = object.edit_facets( parent = editor, view = view,
                                         kind   = 'subpanel' )
            ...
            # Force the contents to be laid out again:
            editor.update()
    ...
    def _menu_rename_node ( self ):
        ...
        if object_label.edit_facets().result:
    ...
    def _menu_new_node ( self, class_name, prompt = False ):
        ...
        if ( not prompt ) or new_object.edit_facets(
    ...
    def _children_replaced ( self, object, facet = '' ):
        ...
        """
        tree = self._tree
        for expanded, node, nid in self._object_info_for( object, facet ):
    ...
    def restore_prefs ( self, prefs ):
        ...
        if self._is_dock_window:
            ...
            self.adapter.layout().sizer.set_structure( self.control, structure )
    ...
    def save_prefs ( self ):
        ...
        if self._is_dock_window:
            return {
                'structure': self.adapter.layout().sizer.get_structure()
            }
...
class ObjectLabel ( HasStrictFacets ):
