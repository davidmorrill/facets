"""
The Qt specific implementations the action manager internal classes.
"""
    ...
    def __init__ ( self, parent, menu, item, controller ):
        ...
        if action.image is None:
            control = menu.addAction(
                action.name, self._side_on_triggered, action.accelerator
            )
        else:
            control = menu.addAction(
                action.image.create_icon(), action.name, self._side_on_triggered,
                action.accelerator
            )
        ...
        if action.style == 'toggle':
            ...
            try:
                ag = item.parent._side_ag
            except AttributeError:
                ag = item.parent._side_ag = QActionGroup( parent )
...
class _Tool ( HasFacets ):
    ...
    def __init__ ( self, parent, tool_bar, image_cache, item, controller,
        ...
        if action.image is None:
            control = tool_bar.addAction( action.name, self._side_on_triggered )
        else:
