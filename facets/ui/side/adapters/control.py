...
from PySide.QtCore \
    import Qt, QObject, QEvent, QPoint, QUrl, QMimeData, SIGNAL

from PySide.QtGui \
    import QCursor, QColor, QPalette, QWidget, QMainWindow, \
           QFontMetrics, QPixmap, QDialog, QApplication, QSizePolicy, \
           QComboBox, QRubberBand, QDrag, QTextEdit, QToolTip
...
from facets.ui.side.pyface.image_resource \
...
class QtControl ( Control ):
    ...
    def _get_value ( self ):
        ...
        ### PYSIDE: if isinstance( result, QString ):
        ### PYSIDE:     return unicode( result )
...
#            self.control.setSelection( start, end - start )
    ...
    def _set_toolbar ( self, toolbar ):
        control = self.control
        getattr( control, '_mw', control ).addToolBar( toolbar )
