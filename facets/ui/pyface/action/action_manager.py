...
from facets.core_api \
    import Bool, Constant, Event, HasFacets, Instance, List, Property, Str

from action_controller \
...
class ActionManager ( HasFacets ):
    """ Abstract base class for all action managers.

        An action manager contains a list of groups, with each group containing
        a list of items.
    ...
    def __init__ ( self, *args, **facets ):
        ...
        super( ActionManager, self ).__init__( **facets )
...
##                     self.insert(-1, group)
    ...
    def destroy ( self ):
        """ Called when the manager is no longer required.
            ...
            By default this method simply calls 'destroy' on all of the
            manager's groups.
        """
    ...
    def insert ( self, index, item ):
        """ Insert an item into the manager at the specified index.
            ...
            2) A string.
                ...
                In which case a 'Group' instance is created with that Id, and
                then inserted into the manager's list of groups.
