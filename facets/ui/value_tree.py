...
from facets.core_api \
    import HasFacets, HasPrivateFacets, Instance, List, Any, Str, Bool
...
class FacetsNode ( ObjectNode ):
    """ A tree node for facets.
    """
    ...
    def tno_when_children_replaced ( self, node, listener, remove ):
        ...
        self.value.on_facet_set( self._children_replaced,
    ...
    def _get_names ( self ):
        """ Gets the names of all defined facets or attributes.
        """
        value = self.value
        names = {}

        for name in value.facet_names( type = lambda x: x != 'event' ):
            names[ name ] = None
...
class _ValueTree ( HasPrivateFacets ):
    ...
    nodes     = [
        ObjectTreeNode(
            node_for = [ NoneNode, StringNode, BoolNode, IntNode, FloatNode,
                         ...
                         ObjectNode, FacetsNode, RootNode, ClassNode ]
        ),
        TreeNode( node_for = [ _ValueTree ],
                  ...
                  copy       = False,
                  label      = '=Values',
                  icon_group = 'facets_node',
                  icon_open  = 'facets_node' )
    ]
)

#-------------------------------------------------------------------------------
#  Defines a 'ValueTree' facet:
#-------------------------------------------------------------------------------

# Facet for a value tree:
