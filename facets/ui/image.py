"""
Defines the ImageLibrary object used to manage Facets UI image libraries.
"""
...
from os \
    import listdir, remove, stat, makedirs, rename, access, pathsep, R_OK, \
           W_OK, X_OK
...
from facets.core_api \
    import HasPrivateFacets, SingletonHasPrivateFacets, Property, Str, Int, \
           List, Dict, File, Instance, Bool, Undefined, FacetError, Float,  \
           Any, cached_property, property_depends_on

from facets.core.facets_env \
    import facets_env

from facets.core.facet_base \
    import get_resource_path, read_file, write_file

from facets.core.facet_db \
    import facet_db

from facets.ui.ui_facets \
    import HasMargin, HasBorder, HorizontalAlignment
...
from facets.lib.resource.resource_reference \
...
image_cache_path = facet_db.db( 'image_cache' )
...
def add_object_prefix ( dict, object, prefix ):
    """ Adds all facets from a specified object to a dictionary with a specified
        name prefix.
    """
...
def split_image_name ( image_name ):
    """ Splits a specified **image_name** into its constituent volume name, file
        name and optional encoded data and returns a tuple of the form:
        ( volume_name, file_name, encoded ).
    """
    col         = image_name.find( ':' )
    ...
    col         = file_name.rfind( '?' )
    if col >= 0:
        encoded   = file_name[ col + 1: ]
        file_name = file_name[ : col ]

    if file_name.find( '.' ) < 0:
        file_name += '.png'

    return ( volume_name, file_name, encoded )
...
class FastZipFile ( HasPrivateFacets ):
    ...
    # The lock used to manage access to the 'zf' facet between the two threads:
    access = Any
...
class ImageInfo ( HasPrivateFacets ):
    """ Defines a class that contains information about a specific Facets UI
        image.
    """
    ...
    # Optional 'encoded' HLSA mask and transform data used to create an
    # ImageInfo object derived from another ImageInfo base object. The value is
    ...
    # The horizontal alignment to use for the label:
    ...
    def clone ( self ):
        ...
        return self.__class__( **self.get(
            'volume', 'image_name', 'description', 'category', 'keywords',
            'width', 'height', 'border', 'content', 'label', 'alignment'
        ) )
...
class ImageVolumeInfo ( HasPrivateFacets ):
    ...
    def _get_image_volume_info_text ( self ):
        ...
        return (ImageVolumeInfoTextTemplate %
               ( description, self.copyright, license, images ))
...
class ImageVolume ( HasPrivateFacets ):
    ...
    # The list of image names available in he volume:
    image_names = Property

    # A dictionary mapping image names to ImageInfo objects:
    catalog = Property

    # A dictionary mapping image names to ImageInfo objects for images with
    # extended 'encoded' data:
    extended_catalog = Property
    ...
    def save ( self ):
        ...
        if not self.is_zip_file:
            ...
            # changed:
            if (modified or
                (image_volume_code != read_file( image_volume_py ))):
        ...
        try:
            ...
            if ((images_code       == cur_zf.read( 'image_info.py' )) and
                (license_text      == cur_zf.read( 'license.txt' ))   and
                (image_volume_code == cur_zf.read( 'image_volume.py' ))):
                return True
        except:
            pass
    ...
    def image_info ( self, image_name ):
        ...
        if info is None:
            info = self.extended_catalog.get( image_name )
            if info is None:
                volume_name, file_name, encoded = split_image_name( image_name )
                if encoded is not None:
                    base_image_name = join_image_name( volume_name, file_name )
                    base_info       = self.catalog.get( base_image_name )
                    if base_info is not None:
                        self.extended_catalog[ image_name ] = info = \
                            base_info.clone().set( encoded = encoded )
    ...
    def image_resource ( self, image_name ):
        ...
        # constructor):
        return ImageResource( file_name ).set( _ref = ref )
    ...
    def _get_image_names ( self ):
        return [ image_info.image_name for image_info in self.images ]

    #-- Property Implementations -----------------------------------------------

    @property_depends_on( 'images' )
    def _get_catalog ( self ):
        return dict( [ ( image.image_name, image ) for image in self.images ] )


    @property_depends_on( 'images' )
    def _get_extended_catalog ( self ):
        return {}
...
class ImageLibrary ( SingletonHasPrivateFacets ):
    """ Manages Facets UI image libraries.
    """
    ...
    def image_info ( self, image_name ):
        ...
        if volume is not None:
            return volume.image_info( image_name )
    ...
    def add_volume ( self, file_name = None ):
        ...
        if isfile( file_name ):
            ...
            raise FacetError(
                ("The add method argument must be None or a file or directory "
                 "path, but '%s' was specified.") % file_name
            )
    ...
    def extract ( self, file_name, image_names ):
        ...
        if exists( file_name ):
            raise FacetError( "The '%s' file already exists." % file_name )
        ...
        try:
            for image_name in set( image_names ):
                ...
                if (image_name[ :1 ] != '@') or (image_name.find( ':' ) < 0):
                    raise FacetError(
                        ("The image name specified by '%s' is not of the "
                         "form: @volume:name.") % image_name
                    )

                # Get the reference volume and image file names:
                image_volume_name, image_file_name, image_encoded = \
                ...
                if image_volume is None:
                    raise FacetError(
                        "Could not find the image volume specified by '%s'." %
                        image_name
                    )
    ...
    def _add_aliases ( self, volume ):
        ...
        for vname in volume.aliases:
            if ((vname in aliases) and (volume_name != aliases[ vname ])):
                raise FacetError(
                    ("Image library error: Attempt to alias '%s' to '%s' when "
                     "it is already aliased to '%s'") %
    ...
    def _duplicate_volume ( self, volume_name ):
        ...
        """
        raise FacetError(
            ("Attempted to add an image volume called '%s' when a volume with "
             "that name is already defined.") % volume_name
        )
