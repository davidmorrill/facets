"""
Defines functions and classes used to create pop-up file dialogs for opening and
saving files.
"""
...
from facets.api \
    import HasPrivateFacets, File, CList, Str, Int, Instance, Property, \
           Button, Bool, Interface, Event, implements, cached_property,\
           View, VGroup, HGroup, VSplit, HSplit, Item, Handler, FileEditor, \
...
class MFileDialogModel ( HasPrivateFacets ):
...
class MFileDialogView ( HasPrivateFacets ):
    """ Defines a visual extension to a file dialog.
    """
...
class ImageInfo ( MFileDialogModel ):
    ...
    def _get_width ( self ):
        try:
            return (str( toolkit().image_size( self._cur_image )[0] ) +
                    ' pixels')
        except:
            return '---'
    ...
    def _get_height ( self ):
        try:
            return (str( toolkit().image_size( self._cur_image )[1] ) +
                    ' pixels')
        except:
            return '---'
...
class OpenFileDialog ( Handler ):
    ...
    # The Facets UI persistence id to use:
    id = Str( 'facets.ui.file_dialog.OpenFileDialog' )
    ...
    def object_create_changed ( self, info ):
        ...
        CreateDirHandler().edit_facets( context = self,
                                        parent  = info.create.control )

    #-- Facets Event Handlers --------------------------------------------------

    def _dclick_set ( self ):
    ...
    def _file_already_exists ( self ):
        ...
        FileExistsHandler(
            message = ("The file '%s' already exists.\nDo you wish to "
                       "overwrite it?") % basename( self.file_name )
        ).edit_facets(
            context = self,
            parent  = self.info.ok.adapter ).set(
            parent  = self.info.ui
        )
...
def open_file ( **facets ):
    """ Returns a file name to open or an empty string if the user cancels the
        operation.
    """
    fd = OpenFileDialog( **facets )
    if fd.edit_facets( view = 'open_file_view' ).result:
        return fd.file_name

    return ''


def save_file ( **facets ):
    ...
    """
    facets.setdefault( 'title', 'Save File' )
    facets[ 'is_save_file' ] = True
    fd = OpenFileDialog( **facets )
    if fd.edit_facets( view = 'open_file_view' ).result:
        return fd.file_name
