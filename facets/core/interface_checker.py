...
MISSING_METHOD = ("The '%s' class does not implement the '%s' method of the "
                  "'%s' interface.")
MISSING_TRAIT  = ("The '%s' class does not implement the %s facet(s) of the "
                  "'%s' interface.")
...
class InterfaceChecker ( HasFacets ):
    ...
    def check_implements ( self, cls, interfaces, error_mode ):
        ...
        # If the class has facets then check that it implements all facets and
        # methods on the specified interfaces:
        if issubclass( cls, HasFacets ):
            for interface in interfaces:
                if not self._check_has_facets_class( cls, interface,
                                                     error_mode ):
                    return False
        ...
        # specified interface:
            ...
            for interface in interfaces:
                if not self._check_non_has_facets_class( cls, interface,
                                                         error_mode ):
                    return False
    ...
    def _check_has_facets_class ( self, cls, interface, error_mode ):
        """ Checks that a 'HasFacets' class implements an interface.
        """
        return ( self._check_facets(  cls, interface, error_mode ) and
                self._check_methods( cls, interface, error_mode ) )


    def _check_non_has_facets_class ( self, cls, interface, error_mode ):
        """ Checks that a non-'HasFacets' class implements an interface.
        """
    ...
    def _check_facets ( self, cls, interface, error_mode ):
        """ Checks that a class implements the facets on an interface.
        """
        missing = set( interface.class_facets() ).difference(
                  set( cls.class_facets() ) )
