...
a base AnImageResource image object by means of an HLSAMask color mask object,
and two HLSATransform color transform objects, one of which is applied to the
masked portion of the base image, and the other to the unmasked part of the
base image.
...
mask and transforms encoded in the string '/H61S25~H61L12S27|L56' applied to it.
In practice, understanding the encoded string contents is not important, since
this is what the HLSADerivedImageEditor helps you create automatically.
...
operation, which inserts the same value shown in the Encoded field. This can be
handy when you are using the HLSADerivedImageEditor to customize an image and
you want to copy the image encoding into your source code.

If you are feeling really adventurous, you can try making changes to the Encoded
value directly. Strings which are legal encodings will result in the
