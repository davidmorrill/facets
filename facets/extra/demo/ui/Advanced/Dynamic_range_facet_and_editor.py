...
You can define a dynamic range using the standard Range facet and specifying
the name of other facets as either the low or high limit value (or both). In
fact, it is even possible to specify the default value of the range facet as
another facet if desired. The facets used as low, high or default values need
not be defined on the same object, but can be defined on any object reachable
from the object (i.e. it allows use of extended facet names).
...
from facets.core_api \
    import HasPrivateFacets, Str, Enum, Range, List, Button, Instance, \
           Property, cached_property, on_facet_set

from facets.api \
    import View, VGroup, HGroup, Item, NotebookEditor, spring

#-- The Hotel class ------------------------------------------------------------

class Hotel ( HasPrivateFacets ):
    ...
    # Event handlers:
    @on_facet_set( 'guests[]' )
    def _guests_modified ( self, added ):
        for guest in added:
            guest.hotel = self

    def _add_guest_set ( self ):
        self.guests.append( Guest() )

#-- The Guest class ------------------------------------------------------------

class Guest ( HasPrivateFacets ):
