...
state of other facets for its value. The cached property provides both a
...
facet listeners that the value of the property has changed.

Normally there is no *setter* for such a property because the value is derived
from the value of other facets.
...
example, the *c* facet is a *settable cached* property whose value must be an
integer. Attempting to set a non-integer value of the property will raise an
exception, just like any other facet would.

If any of the facets which the property depends upon change value, the current
...
In the example, facet *c* is a *settable cached* property which returns the
product of *a* times *b*, and facet *d* is a normal *cached* property that
returns double the value of *c*.

To see the effect of these facets in action, try moving the *a* and *b* sliders
and watching the *c* and *d* facets update. This demonstrates how *c* and *d*
are properties that depend upon the values of other facets.

Now try changing the value of the *c* facet by moving the slider or typing a
new value into the text entry field. You will see that the *d* facet updates as
well, illustrating that the *c* facet can be set directly, as well as indirectly
by changes to *a* and *b*.
...
Now try typing a value into the *d* facet and you will see that an error results
(indicated by the text entry field turning red), because this is a normal
*cached* facet that has no *setter* method defined.
"""
...
from facets.core_api \
    import HasFacets, Int, Range, Property, property_depends_on
...
class SettableCachedProperty ( HasFacets ):
    ...
    view = View(
        ...
        Item( 'd',
              style  = 'readonly',
              editor = RangeEditor( low = 1, high = 400 ) ),
        Item( 'd' ),
        width = 0.3
    )
