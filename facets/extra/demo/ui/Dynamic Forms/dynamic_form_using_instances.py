...
from facets.core_api \
    import HasFacets, Str, Range, Enum, Bool, Instance

from facets.api \
    import Item, Group, View, Handler


class Spec ( HasFacets ):
    """ An empty class from which all age-specific facet list classes are
        derived.
    """
    pass
...
class ChildSpec ( Spec ):
    """ Facet list for children (assigned to 'misc' for a Person when age < 18).
    """
    legal_guardian = Str
    school         = Str
    grade          = Range( 1, 12 )

    facets_view = View( 'legal_guardian',
                        'school',
                        'grade' )


class AdultSpec ( Spec ):
    """ Facet list for adults (assigned to 'misc' for a Person when age >= 18).
    """
    ...
    facets_view = View( 'marital_status',
                        'registered_voter',
                        'military_service' )
