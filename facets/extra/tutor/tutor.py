...
from facets.api \
    import HasPrivateFacets, HasFacets, File, Directory, Instance, Int, Str, \
           List, Bool, Dict, Property, Event, cached_property, View, VGroup, \
           HGroup, VSplit, HSplit, Tabbed, Item, Heading, Handler,           \
           NotebookEditor, CodeEditor, HTMLEditor, TreeEditor, TitleEditor,  \
           ValueEditor, VIPShellEditor, ThemedButtonEditor
...
try:
    from facets.ui.wx.editors.extra.windows.ie_html_editor \
        import IEHTMLEditor

    from facets.ui.wx.editors.extra.windows.flash_editor \
        import FlashEditor
except:
...
<body>
<p><object classid="clsid:22D6F312-B0F6-11D0-94AB-0080C74C7E95"
...
<embed src="%s" AutoStart="true" ShowTracker="true" ShowControls="true"
ShowGotoBar="false" ShowDisplay="false" ShowStatusBar="false" AutoSize="true"
...
<embed src="file:///%s" width="100%%" height="100%%" scale="aspect"
...
class DemoPane ( HasPrivateFacets ):
    """ Displays the contents of a Python lab's *demo* value.
    """

    #-- Facet Definitions ------------------------------------------------------

    demo = Instance( HasFacets, factory = NoDemo )
    ...
    view = View(
        ...
        id        = 'facets.extra.info.tutorials.demo',
        resizable = True
    )
...
class ATutorialItem ( HasPrivateFacets ):
...
class HTMLItem ( ADescriptionItem ):
    """ Defines a class used for displaying a single HTML page within the tutor
        using the default Facets HTML editor.
    """
...
class HTMLStrItem ( HTMLItem ):
    """ Defines a class used for displaying a single HTML text string within
        the tutor using the default Facets HTML editor.
    """

    # Make the content a real facet rather than a property:
    content = Str


# Create the Internet Explorer support classes (if available):
if IEHTMLEditor is not None:
    ...
    class IEHTMLItem ( HTMLItem ):
        """ Defines a class used for displaying a single HTML page within the
            tutor using the Facets Internet Explorer HTML editor.
        """
    ...
    class IEHTMLStrItem ( IEHTMLItem ):
        """ Defines a class used for displaying a single HTML text string within
            the tutor using the Facets Internet Explorer HTML editor.
        """
...
# Create the Flash support class (if available):
if FlashEditor is not None:
...
class ASection ( HasPrivateFacets ):
...
class Lecture ( ASection ):
    ...
    view = View(
        ...
        id = 'facets.extra.info.tutorials.lecture'
    )
...
class Lab ( ASection ):
    ...
    view = View(
        ...
        id      = 'facets.extra.info.tutorials.lab',
        handler = LabHandler
    )
        ...
        try:
            try:
                ...
                for name in ( 'demo', 'popup' ):
                    if isinstance( values.get( name ), HasFacets ):
                        del values[ name ]

                # Execute the current lab code:
                exec module[ 2: ] in values, values

                # fixme: Hack trying to update the Facets UI view of the dict.
                self.values = {}
                self.values = values
                ...
                if isinstance( popup, HasFacets ):
                    popup.edit_facets( kind = 'livemodal' )
...
class Lesson ( Lab ):
    ...
    view = View(
        ...
        id      = 'facets.extra.info.tutorials.lesson',
        handler = LabHandler
    )
...
class SectionFactory ( HasPrivateFacets ):
    ...
    def _create_html_item ( self, **facets ):
        """ Creates a platform specific HTML item and adds it to the list of
            descriptions.
        """
        if IEHTMLEditor is not None:
            item = IEHTMLItem( **facets )
        else:
            item = HTMLItem( **facets )
...
if __name__ == '__main__':
    ...
    if tutor.root is not None:
        ...
        print """No facets tutorial found in %s.

Correct usage is: python tutor.py [tutorial_path]
where: tutorial_path = Path to the root of the facets tutorial.
